...
export class EditorView {
  ...
  ///
  /// @cn 当将现有的编辑器视图移动到新的 document 或shadow root tree 中时，调用此方法以重新计算其根。
  updateRoot() {
    this._root = null
  }
  ...
  /// 如果该位置在两个并不直接相邻的元素之间，则 `side` 参数决定了这两个元素哪个元素被使用，当 `side` 小于 0 的时候，使用位置前面（左边）的元素，否则是后面的元素。
  ///
  /// @comment 光标只有高度没有宽度，因此只有 top 和 bottom 及 height 信息；left 和 right 总是一样的，width 总是 0.
  ///
  /// @comment 这个方法也很常用，不过一般情况下你不会用到 DOM 的坐标信息。
  coordsAtPos(
    pos: number,
    side = 1
  ...
  ///
  /// @cn 返回给定位置的 DOM 节点。如果 `side` 参数是负的，则寻找离 position 最近的前面（左边的元素）
  /// 否则就寻找离位置最近的右边的元素。如果是 0，则会更倾向于返回一个较浅（？）的位置。
  ///
  /// Note that you should **not** mutate the editor's internal DOM,
  /// only inspect it (and even that is usually not necessary).
  ///

  /// @cn 记住：你 **绝对不应该** 直接修改编辑器内部的 DOM，而只能查看它（虽然即使是检查它也是不必要的）
  ///
  ...
  ///
  /// @cn 使用给定的HTML字符串运行编辑器的粘贴逻辑。如果提供了event，将传递给[`handlePaste`](#view.EditorProps.handlePaste)钩子函数。
  ...
  /// used anymore).
  ///
  /// @cn 当 view 已被[destroyed](#view.EditorView.destroy)时，这个条件为 true（因此 view 在这种情况都不应该再次使用）。
  get isDestroyed() {
...
/// [plugin](#state.Plugin).
///
/// @cn 可选的 type 参数用于指定 prop 函数中的 this 类型，并在定义 plugin 时传递插件类型。
export interface EditorProps<P = any> {
  ...
  ///
  /// @cn 可以用来在将粘贴的内容或者拖拽过来的内容应用到文档之前转换一下。
  ...
  /// inner decorations.
  ///
  ///   @cn `innerDecorations` 指向节点内容的装饰器。如果你的 view 没有内容或者没有 `contentDOM` 属性的话，你可以安全的忽略它，因为
  ///   编辑器将会把该 decorations 绘制到其内容上。不过如果你的 view 有内容，比如，用该内容创建一个嵌套的 editor，那么提供给其内部元素的内部 decorations 就是有用的。
  ///
  /// (For backwards compatibility reasons, [mark
  /// views](#view.EditorProps.markViews) can also be included in this
  /// object.)
  ///
  /// 出于向后兼容的考虑，此对象中还可以包含 [mark
  /// views](#view.EditorProps.markViews)
  ...
  /// is inline.
  ///
  /// @cn 传递自定义 mark 渲染函数。请注意，这些函数无法像 [node views](#view.NodeView) 那样提供动态行为，它们只提供自定义渲染逻辑。
  /// 第三个参数标记 mark 的内容是否为内联。
  ...
  /// 这个对象只会调用它的 [`serializeFragment`](#model.DOMSerializer.serializeFragment) 方法，你可以提供一个实现兼容方法的替代对象类型。
  ...
  ///
  ///   @comment 之前该方法返回 DecorationSet，现在返回 DecorationSource，有兴趣的可以研究下区别。
  decorations?: (
    this: P,
    state: EditorState
...
export interface DirectEditorProps extends EditorProps {
  ...
  ///
  /// @cn 一组可用于 view 中的插件，它们的[plugin view](#state.PluginSpec.view)和[props](#state.PluginSpec.props)会被作用在当前的 view 上面
  /// 如果传递具有状态组件（[state field](#state.PluginSpec.state) 或 [transaction](#state.PluginSpec.filterTransaction)）的插件，将会出现错误，
  /// 因为这些插件必须存在于 state 中才能正常工作。
